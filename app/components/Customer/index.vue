  ...
  <div class="flex flex-col w-full gap-2">
    ...
    <div v-else class="flex flex-col gap-2 pb-2">
      ...
      <div v-if="item.contact && item.contact.length" class="flex flex-col gap-2">
        ...
        <div 
          v-for="(contactPerson, index) in item.contact" :key="index" 
          class="grid gap-3 bg-gray-200 dark:bg-gray-900 rounded-lg p-3">
          <div v-if="contactPerson.name || contactPerson.position" class="flex flex-col gap-1 text-gray-900 dark:text-white">
            <span class="text-sm opacity-70">{{ contactPerson.position || 'Контактное лицо' }}</span>
            <span class="font-bold">{{ contactPerson.name }}</span>
          </div>
          <div v-if="contactPerson.phone" class="flex flex-col gap-1 text-gray-900 dark:text-white">
            <span class="text-sm opacity-70">Телефон</span>
            <span class="font-bold">{{ contactPerson.phone }}</span>
          </div>
          <div v-if="contactPerson.email" class="flex flex-col gap-1 text-gray-900 dark:text-white">
            <span class="text-sm opacity-70">Электронный адрес</span>
            <span class="font-bold">{{ contactPerson.email }}</span>
          </div>
        </div>
      </div>
