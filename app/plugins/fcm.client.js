...
export default defineNuxtPlugin(async () => {
  ...
  const vapidKey =
    publicRuntime.NUXT_PUBLIC_FCM_VAPID_KEY || import.meta.env.NUXT_PUBLIC_FCM_VAPID_KEY
  ...
  // 3) Нужен именно ваш SW в scope /app/
  async function getRegistration() {
    if (!('serviceWorker' in navigator)) return null
    // дождемся регистратора именно вашего sw.ts (injectManifest)
    const reg = await navigator.serviceWorker.ready
    // sanity-check: SW должен контролировать /app/
